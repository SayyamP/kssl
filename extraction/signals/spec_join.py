...
Three separate defects, all in the same join:
  ...
  2. THE BORE RULE WAS APPLIED TO THE ROW, NOT TO THE FIELD. client_portfolio.kssl_side
     refuses EVERY label except "Calibre" when the two products' bores differ. That rule
     is right for artillery -- a 105 mm gun's range is not a figure to set beside a
     155 mm gun's -- and it is far too wide: the CQB Carbine's 3.15 kg, its 508/407/360
     mm barrel and its gas-operated bullpup action do not become unpublishable because
     the AK-203 fires a 7.62 mm cartridge. Measured, that refusal alone removed four of
     the five archive labels on matchup 20102. BORE_SENSITIVE below states which fields
     the bore actually governs; the rest are joined.
...
# neither can be scored as a lead; leaving them out also keeps kssl_side behaving
# exactly as it did, where "Calibre" was the single label the old rule let through.
    ...
    for fk, e in cf.items():
        ...
        if cur is not None:
            ...
            if not stated(cur.get("kv")):
                ...
                if (e["base"] is not None and cur.get("cn") is not None
...
def _demo():
    ...
    # -- the bore rule, per field
    got, rep = join([], prow, bore_differs=True)
    ...
    ck("...and does NOT refuse the weight", "Weight" in keys)
